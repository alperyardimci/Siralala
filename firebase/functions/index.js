...
app.post("/api/users/register", async (req, res) => {
  ...
  const ref = await db.collection("users").add({
    username, display_name: displayName, friend_code: code, created_at: new Date().toISOString()
  });
...
app.get("/api/users/search", async (req, res) => {
  ...
  const doc = snap.docs[0];
  const d = doc.data();
  res.json({ id: doc.id, username: d.username, display_name: d.display_name, friend_code: d.friend_code });
});
...
app.post("/api/friends/add", async (req, res) => {
  ...
  await col.doc(`${user.id}_${friend.id}`).set({ user_id: user.id, friend_id: friend.id });
  await col.doc(`${friend.id}_${user.id}`).set({ user_id: friend.id, friend_id: user.id });
...
app.get("/api/friends", async (req, res) => {
  ...
  const friendIds = snap.docs.map(d => d.data().friend_id);
  ...
  for (const fid of friendIds) {
    ...
    if (fdoc.exists) {
      const d = fdoc.data();
      friends.push({ id: fdoc.id, username: d.username, display_name: d.display_name });
    }
  }
  res.json(friends);
});
...
app.delete("/api/friends/:friendId", async (req, res) => {
  ...
  await db.collection("friendships").doc(`${user.id}_${fid}`).delete();
  await db.collection("friendships").doc(`${fid}_${user.id}`).delete();
...
app.post("/api/groups", async (req, res) => {
  ...
  const ref = await db.collection("groups").add({
    name, owner_id: user.id, created_at: new Date().toISOString()
  });
  ...
  if (memberUsernames) {
    for (const mu of memberUsernames) {
      const member = await getUser(mu);
      if (member) {
        await db.collection("group_members").doc(`${ref.id}_${member.id}`).set({ group_id: ref.id, user_id: member.id });
        members.push({ id: member.id, username: member.username, display_name: member.display_name });
      }
    }
  }
...
app.get("/api/groups", async (req, res) => {
  ...
  for (const gid of groupIds) {
    ...
    for (const md of mSnap.docs) {
      const udoc = await db.collection("users").doc(md.data().user_id).get();
      if (udoc.exists) {
        const u = udoc.data();
        members.push({ id: udoc.id, username: u.username, display_name: u.display_name });
      }
    }
    groups.push({ id: gid, name: g.name, owner_id: g.owner_id, members });
  }
  res.json(groups);
});
...
app.post("/api/groups/:groupId/members", async (req, res) => {
  const member = await getUser(req.body.username);
  if (!member) return res.status(404).json({ error: "User not found" });
  await db.collection("group_members").doc(`${req.params.groupId}_${member.id}`).set({
    group_id: req.params.groupId, user_id: member.id
  });
  res.json({ success: true });
});
...
app.post("/api/pools/share", async (req, res) => {
  ...
  const ref = await db.collection("shared_pools").add({
    group_id: groupId, creator_id: user.id, name, item_count: items.length,
    created_at: new Date().toISOString()
  });

  for (let i = 0; i < items.length; i++) {
    await db.collection("shared_pool_items").add({
      pool_id: ref.id, name: items[i].name, image_data: items[i].imageData || null, sort_order: i
    });
  }
...
app.get("/api/groups/:groupId/pools", async (req, res) => {
  ...
  for (const doc of snap.docs) {
    const d = doc.data();
    const udoc = await db.collection("users").doc(d.creator_id).get();
    pools.push({
      id: doc.id, name: d.name, group_id: d.group_id, creator_id: d.creator_id,
...
app.get("/api/pools/shared", async (req, res) => {
  ...
  for (const gid of groupIds) {
    ...
    const gdoc = await db.collection("groups").doc(gid).get();
    const groupName = gdoc.exists ? gdoc.data().name : "?";

    for (const doc of pSnap.docs) {
      const d = doc.data();
      const udoc = await db.collection("users").doc(d.creator_id).get();
      pools.push({
        id: doc.id, name: d.name, group_id: gid, creator_id: d.creator_id,
        creator_name: udoc.exists ? udoc.data().display_name : "?",
        group_name: groupName, item_count: d.item_count || 0
      });
    }
  }
  res.json(pools);
...
app.delete("/api/pools/:poolId", async (req, res) => {
  ...
  if (!doc.exists || doc.data().creator_id !== user.id) return res.status(403).json({ error: "Only owner can delete" });
...
app.post("/api/questions", async (req, res) => {
  ...
  const ref = await db.collection("shared_questions").add({
    group_id: groupId, creator_id: user.id, text, pool_name: poolName,
    item_count: count, is_active: true, created_at: new Date().toISOString()
  });
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    await db.collection("shared_question_items").add({
      question_id: ref.id, name: itemName, image_data: itemImage, sort_order: i
    });
  }
...
app.get("/api/questions/pending", async (req, res) => {
  ...
  for (const gid of groupIds) {
    ...
    for (const qDoc of qSnap.docs) {
      ...
      questions.push({
        id: qDoc.id, text: q.text, pool_name: q.pool_name, item_count: q.item_count,
        creator_name: udoc.exists ? udoc.data().display_name : "?", group_name: groupName,
...
app.get("/api/questions/completed", async (req, res) => {
  ...
  for (const rDoc of rankedSnap.docs) {
    ...
    const itemsSnap = await db.collection("shared_question_items").where("question_id", "==", qid).orderBy("sort_order").get();
    const allRanked = await db.collection("rankings").where("question_id", "==", qid).get();

    questions.push({
      id: qid, text: q.text, pool_name: q.pool_name, item_count: q.item_count,
      creator_name: udoc.exists ? udoc.data().display_name : "?",
      group_name: gdoc.exists ? gdoc.data().name : "?",
      items: itemsSnap.docs.map(d => ({ id: d.id, ...d.data() })),
      completion_count: allRanked.size
    });
  }
  res.json(questions);
});
...
app.get("/api/groups/:groupId/questions", async (req, res) => {
  ...
  const gdoc = await db.collection("groups").doc(req.params.groupId).get();
  const groupName = gdoc.exists ? gdoc.data().name : "?";
  ...
  for (const qDoc of qSnap.docs) {
    ...
    const itemsSnap = await db.collection("shared_question_items").where("question_id", "==", qDoc.id).orderBy("sort_order").get();
    const rSnap = await db.collection("rankings").where("question_id", "==", qDoc.id).get();
    const userRanked = rSnap.docs.some(d => d.data().user_id === user.id);
    ...
    questions.push({
      ...
      creator_name: udoc.exists ? udoc.data().display_name : "?", group_name: groupName,
      items: itemsSnap.docs.map(d => ({ id: d.id, ...d.data() })),
      completion_count: rSnap.size, user_ranked: userRanked
    });
  }
  res.json(questions);
});
...
app.post("/api/questions/:questionId/dismiss", async (req, res) => {
  ...
  if (!user) return res.status(404).json({ error: "User not found" });
  await db.collection("dismissed_questions").doc(`${user.id}_${req.params.questionId}`).set({
    user_id: user.id, question_id: req.params.questionId
  });
  res.json({ success: true });
});
...
app.post("/api/rankings", async (req, res) => {
  ...
  const existing = await db.collection("rankings")
    .where("question_id", "==", questionId).where("user_id", "==", user.id).limit(1).get();
  if (!existing.empty) return res.status(400).json({ error: "Already ranked" });

  const ref = await db.collection("rankings").add({
    question_id: questionId, user_id: user.id, completed_at: new Date().toISOString()
  });

  for (const e of entries) {
    await db.collection("ranking_entries").add({
      ranking_id: ref.id, item_id: e.itemId, rank: e.rank
    });
  }

  res.json({ id: ref.id, question_id: questionId });
});

app.get("/api/rankings", async (req, res) => {
  const questionId = req.query.questionId;
  const rSnap = await db.collection("rankings").where("question_id", "==", questionId).get();
  ...
  for (const rDoc of rSnap.docs) {
    ...
    for (const eDoc of eSnap.docs) {
      const e = eDoc.data();
      const itemDoc = await db.collection("shared_question_items").doc(e.item_id).get();
      const itemData = itemDoc.exists ? itemDoc.data() : {};
      entries.push({
        rank: e.rank, item_id: e.item_id,
        item_name: itemData.name || "?", item_image: itemData.image_data || null
      });
    }

    rankings.push({
      id: rDoc.id, participant_name: udoc.exists ? udoc.data().display_name : "?",
      completed_at: r.completed_at, entries
    });
  }
  res.json(rankings);
});

exports.api = onRequest({ region: "europe-west1", memory: "256MiB", timeoutSeconds: 60 }, app);
